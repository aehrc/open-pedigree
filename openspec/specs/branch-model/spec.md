...
## Purpose
Define the repository's git branching model as it migrates from git-flow
(`master`/`develop`) to a trunk-based workflow with a single long-lived
`main` branch, matching the pattern adopted by the org's other trunk-based
modules.
