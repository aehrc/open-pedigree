...
Lets an implementer configure additional per-individual data fields for the pedigree editor by pointing at a FHIR `Questionnaire` resource, instead of patching the editor's source. Supports a flat-plus-groups subset of the Questionnaire item model, conditional visibility via `enableWhen`, terminology-backed choice fields, and round-tripping answers through GA4GH FHIR export/import as contained `QuestionnaireResponse` resources — including optional mapping of individual items onto existing scalar node properties or coded `Condition`/`Observation` resources so the data lands in its natural FHIR home rather than only in a generic `QuestionnaireResponse`.
