...
**Implementation note added during `/code-review` (resolves an ambiguity found during adversarial review):** `openEditor`'s only argument beyond `nodeId` is `onDone` — it deliberately does *not* also receive the node's current `recordRef`, even though a concrete provider (e.g. a REDCap-backed one) needs to know *which* linked record to open. This is intentional, not an oversight: `canLink(nodeId)`/`canCreateNew(nodeId)` already establish a nodeId-only shape, and a provider that needs the current ref reads it itself via `window.editor.getView().getNode(nodeId).getLinkedRecordRef()` — exactly the pattern `SmartPatientProvider.ts` already uses to read node state from inside a concrete provider (`(window as any).editor.getView().getNode(probandNodeId)`). This was considered against the alternative of adding `recordRef` as an explicit third parameter (mirroring `AbstractPatientProvider.openClinicalImportModal(nodeId, patientRef, onImported)`'s existing precedent) and rejected in favor of keeping all three nodeId-taking methods uniform, since the ref is already reachable with zero interface change.
