...
- **New capability**: `record-link-provider` — a new `AbstractRecordLinkProvider` (in `src/script/recordLinkProvider/`), a sibling to `AbstractPatientProvider`, not a modification of it:
  ...
  - `createNew(nodeId, onCreated)` — create a brand-new linked record when none exists yet. `onCreated(recordRef, answers)` mirrors `onLinked`'s `recordRef` argument (a newly-created record has its own ref the instant it exists, and the editor needs it immediately to set the node's `linkedRecordRef` — otherwise the node could never reach `canEditLinkedRecord`/the Edit action afterward), plus the same answer-bag shape `onDone` uses for the rest.
  - `onDone`'s `answers` and `onCreated`'s `answers` reuse the exact `{linkId: string, value: any}[]` answer-bag shape `generalize-patient-provider-import` already established for `onImported`, and dispatch through the same existing `_resolveQuestionnaireSetter` priority — no second dispatch mechanism.
