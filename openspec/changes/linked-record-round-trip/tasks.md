...
- [x] 1.1 `Person`: add the record snapshot beside `_linkedRecordRef` (D3); reset it when the ref changes; round-trip it in `getProperties`/`assignProperties` (`internal` format)
...
- [x] 2.2 `_dispatchLinkedRecordRefresh` (D4): compare with the snapshot (revised D4): set changes, clear only while the node still holds the record's value, reconcile legends; one flagged event with only real changes plus the new snapshot; nothing when unchanged; put back setter side effects once (D7)
