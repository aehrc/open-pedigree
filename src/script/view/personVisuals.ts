...
export default class PersonVisuals extends AbstractPersonVisuals {
    _nameLabel: any;
    _stillBirthLabel: any;
    _ageLabel: any;
    _externalIDLabel: any;
    _commentsLabel: any;
    _childlessStatusLabel: any;
    _disorderShapes: any;
    _deadShape: any;
    _unbornShape: any;
    _childlessShape: any;
    ...
    // ChildlessBehaviorVisuals mixin methods (added below)
    getChildlessShape: any;
    getChildlessStatusLabel: any;
    updateChildlessShapes: any;
    updateChildlessStatusLabel: any;
    ...
    updateDisorderShapes(): any {
        ...
        var gradient = function(color: any, angle: any) {
        ...
        if (this.getNode().getLifeStatus() == 'aborted' || this.getNode().getLifeStatus() == 'miscarriage') {
            ...
            var radius2 = (this._shapeRadius-0.6);    // -0.6 to avoid disorder fills to overlap with shape borders (due to aliasing/Raphael pixel layout)
            if (this.getNode().getGender() == 'U') {
                radius2 *= 1.155;
            ...
            for(var i = 0; i < colors.length; i++) {
                ...
                disorderShapes.push(sector(editor.getPaper(), this.getX(), this.getY(), radius2,
    ...
    drawDeadShape(): any {
        ...
        if(this.getNode().getLifeStatus() == 'aborted') {
            ...
            var deadX = this.getX() - height/1.5;
            if (this.getNode().isPersonGroup()) {
                deadX -= PedigreeEditorParameters.attributes.radius/4;
            }

            var deadY = this.getY() + height/3;
            this._deadShape = editor.getPaper().path(['M', deadX, deadY, 'l', height + height/3, -(height+ height/3), 'z']);
    ...
    updateAgeLabel(): any {
        ...
        if (person.isFetus()) {
            ...
            if (person.getBirthDate()) {
                var age: any = getAge(person.getBirthDate(), null);
            ...
            if(person.getDeathDate() && person.getBirthDate()) {
                var age2: any = getAge(person.getBirthDate(), person.getDeathDate());
                if (age2.indexOf('day') != -1 || age2.indexOf('wk') != -1 || age2.indexOf('mo') != -1) {
                    text = 'd. ' + person.getDeathDate().getFullYear() + ' (' + age2 + ')';
                } else {
     ...
     */
    updateLifeStatusShapes(oldStatus: any): any {
     ...
     * @method setSelected
     */
    setSelected(isSelected: any): any {
    ...
    getShapes(): any {
        ...
        return super.getShapes().concat(editor.getPaper().set(this.getDisorderShapes(), lifeStatusShapes));
    }
    ...
    getAllGraphics(): any {
        ...
        return super.getAllGraphics().push(this.getHoverBox().getBackElements(), this.getLabels(), this.getCarrierGraphics(), this.getEvaluationGraphics(), this.getHoverBox().getFrontElements());
    }
    ...
    setPos(x: any, y: any, animate: any, callback: any): any {
        ...
        super.setPos(x, y, animate, funct);
    }
}

//ATTACHES CHILDLESS BEHAVIOR METHODS
Object.assign(PersonVisuals.prototype, ChildlessBehaviorVisuals);
