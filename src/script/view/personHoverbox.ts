...
export default class PersonHoverbox extends AbstractHoverbox {
    ...
    generateHandles(): any {
        ...
        if (!node.isFetus()) {
            ...
            if (node.getChildlessStatus() === null) {
                ...
                var path2 = [['M', x, y],['L', x, y+PedigreeEditorParameters.attributes.personHandleBreakX]];
                editor.getPaper().path(path2).attr({'stroke-width': strokeWidth, stroke: 'gray'}).insertBefore(nodeShapes);
            ...
            var path3 = [['M', x, vertPosForPartnerHandles],['L', x + PedigreeEditorParameters.attributes.personHandleBreakX, vertPosForPartnerHandles]];
            editor.getPaper().path(path3).attr({'stroke-width': strokeWidth, stroke: 'gray'}).insertBefore(nodeShapes);
    ...
    handleAction(handleType: any, isDrag: any, curHoveredId: any): any {
        ...
        if(isDrag && curHoveredId !== null) {
            ...
            if(handleType == 'parent') {
                ...
                this.removeHandles();
                var event2: any = { 'personID': this.getNode().getID(), 'partnerID': curHoveredId };
                document.dispatchEvent(new CustomEvent('pedigree:person:drag:newpartner', { detail: event2 }));
            } else if(handleType == 'child') {
                var event3: any = { 'personID': curHoveredId, 'parentID': this.getNode().getID() };
                document.dispatchEvent(new CustomEvent('pedigree:person:drag:newparent', { detail: event3 }));
            } else if(handleType == 'sibling') {
                var event4: any = { 'sibling2ID': curHoveredId, 'sibling1ID': this.getNode().getID() };
                document.dispatchEvent(new CustomEvent('pedigree:person:drag:newsibling', { detail: event4 }));
            }
        } else if (!isDrag) {
            if(handleType == 'partnerR' || handleType == 'partnerL') {
                this.removeHandles();
                var preferLeft = (this.getNode().getGender() == 'F') || (handleType == 'partnerL');
                var event5: any = { 'personID': this.getNode().getID(), 'preferLeft': preferLeft };
                document.dispatchEvent(new CustomEvent('pedigree:person:newpartnerandchild', { detail: event5 }));
            } else if(handleType == 'child') {
                ...
                var position2 = editor.getWorkspace().canvasToDiv(this.getNodeX() - PedigreeEditorParameters.attributes.personSiblingHandleLengthX,
                ...
                editor.getSiblingSelectionBubble().show(this.getNode(), position2.x, position2.y);
            } else if(handleType == 'parent') {
                this.removeHandles();
                this.removeButtons();
                var event6: any = { 'personID': this.getNode().getID() };
                document.dispatchEvent(new CustomEvent('pedigree:person:newparent', { detail: event6 }));
            }
        }
