...
export default class NodetypeSelectionBubble {
    ...
    handleCreateAction(data: any): any {
        ...
        if (nodeType == 'Person') {
            var evt = { 'personID': id, 'childParams': data.params.parameters, 'preferLeft': false } as any;
            if (data.params.twins) {
                evt['twins'] = this.numTwinNodes;
            }
            if (data.params.group) {
                evt['groupSize'] = this.numPersonsInGroup;
            }
