...
export default class LineSet {
    ...
    replaceIDs(changedIdsSet: any): any {
        var newLineCrossings = {} as any;
        ...
        for (var i = 0; i < this._lines.length; i++) {
            var oldLineID = this._lines[i].owner;
            var newLineID = changedIdsSet.hasOwnProperty(oldLineID) ? changedIdsSet[oldLineID] : oldLineID;
            this._lines[i].owner = newLineID;
        }
    }
    ...
    removeAllLinesAffectedByOwnerMovement(owner: any): any {
        var returnNewAffected = [] as any[];

        var processed = {} as any;
