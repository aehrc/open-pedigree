...
export default class ImportSelector {
  ...
  constructor() {
    ...
    var promptImport = new (Element as any)('div', {'class': 'import-section'}).update('Import data:');
    this.importValue = new (Element as any)('textarea', {'id': 'import', 'value': '', 'class': 'import-textarea'});
    ...
    if (!!(window as any).FileReader && !!(window as any).FileList) {
      ...
      var uploadFileSelector = new (Element as any)('input', {'type' : 'file', 'id': 'pedigreeInputFile', 'style': 'display:none'});
      uploadFileSelector.observe('change', function(this: any, event: any) {
      ...
      var uploadLink = new (Element as any)('div', {'class': 'import-upload'}).update('(<a>Select a local file to be imported</a>)');
      uploadLink.observe('click', function(event: any) {
    ...
    var _addTypeOption = function (checked: any, labelText: any, value: any) {
      ...
      var input = new (Element as any)('input', {'type' : 'radio', 'value': value, 'name': 'select-type'});
    ...
    var _addConfigOption = function (checked: any, labelText: any, value: any) {
      var optionWrapper = new (Element as any)('tr');
      var input = new (Element as any)('input', {'type' : 'radio', 'value': value, 'name': 'select-options' });
      if (checked) {
        (input as any).checked = true;
      }
      var label = new (Element as any)('label', {'class': 'import-config-label'}).insert(input).insert(labelText);
      optionWrapper.insert(label.wrap('td'));
      return optionWrapper;
    };
    var configListElement = new (Element as any)('table', {id : 'import-type'});
    ...
    var markEvaluated = new (Element as any)('input', {'type' : 'checkbox', 'value': '1', 'name': 'mark-evaluated'});
    var markLabel1     = new (Element as any)('label', {'class': 'import-mark-label1'}).insert(markEvaluated).insert('Mark all patients with known disorder status with \'documented evaluation\' mark').wrap('td').wrap('tr');
    configListElement.insert(markLabel1);
    var markExternal = new (Element as any)('input', {'type' : 'checkbox', 'value': '1', 'name': 'mark-external'});
    (markExternal as any).checked = true;
    var markLabel2   = new (Element as any)('label', {'class': 'import-mark-label2'}).insert(markExternal).insert('Save individual IDs as given in the input data as \'external ID\'').wrap('td').wrap('tr');
    ...
    buttons.insert(new (Element as any)('input', {type: 'button', name : 'import', 'value': 'Import', 'class' : 'button', 'id': 'import_button'}).wrap('span', {'class' : 'buttonwrapper'}));
    ...
    importButton.observe('click', function(event: any) {
      _this._onImportStarted();
    });

    var closeShortcut = ['Esc'];
    this.dialog = new (window as any).PhenoTips.widgets.ModalPopup(mainDiv, {close: {method : this.hide.bind(this), keys : closeShortcut}}, {extraClassName: 'pedigree-import-chooser', title: 'Pedigree import', displayCloseButton: true});
  }
  ...
  disableEnableOptions(): any {
    var importType = ($$('input:checked[type=radio][name="select-type"]') as any)[0].value;
    //console.log("Import type: " + importType);
    var pedOnlyOptions = $$('input[type=radio][name="select-options"]');
    for (var i = 0; i < pedOnlyOptions.length; i++) {
      if (importType != 'ped') {
        (pedOnlyOptions[i] as any).disabled = true;
      } else {
        (pedOnlyOptions[i] as any).disabled = false;
      }
    }
    var pedAndGedcomOption = ($$('input[type=checkbox][name="mark-evaluated"]') as any)[0];
    ...
    var saveExternalID = ($$('input[type=checkbox][name="mark-external"]') as any)[0];
    if (importType != 'DADA2'){
  ...
  _onImportStarted(): any {
    ...
    var importMark = ($$('input[type=checkbox][name="mark-evaluated"]') as any)[0].checked;

    var externalIdMark = ($$('input[type=checkbox][name="mark-external"]') as any)[0].checked;

    var optionSelected = ($$('input:checked[type=radio][name="select-options"]') as any)[0].value;
