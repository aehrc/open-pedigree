...
export default class ImportSelector {
    ...
    constructor() {
        ...
        if (!!window.FileReader && !!(window as any).FileList) {
    ...
    disableEnableOptions(): any {
        var importType = ($$('input:checked[type=radio][name="select-type"]')[0] as any).value;
        ...
        if (importType != 'ped' && importType != 'gedcom') {
            (pedAndGedcomOption as any).disabled = true;
        } else {
            (pedAndGedcomOption as any).disabled = false;
        }

        var saveExternalID = $$('input[type=checkbox][name="mark-external"]')[0];
        (saveExternalID as any).disabled = false;
    }
    ...
    _onImportStarted(): any {
        ...
        var importMark = ($$('input[type=checkbox][name="mark-evaluated"]')[0] as any).checked;

        var externalIdMark = ($$('input[type=checkbox][name="mark-external"]')[0] as any).checked;

        var optionSelected = ($$('input:checked[type=radio][name="select-options"]')[0] as any).value;
