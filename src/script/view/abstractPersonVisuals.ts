...
export default class AbstractPersonVisuals extends AbstractNodeVisuals {
     ...
     */
    getShapes(): any {
        var shapes = super.getShapes().push(this.getGenderGraphics());
    ...
    setGenderGraphics(): any {
        ...
        if (!editor.isUnsupportedBrowser()) {
            shadow = shape.clone().attr({stroke: 'none', fill: 'gray', opacity: .3});
