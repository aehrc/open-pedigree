...
export default class Partnership extends AbstractNode {
    ...
    constructor(x: any, y: any, id: any, properties: any) {
        // Set fields before super() calls _generateGraphics
        (Partnership as any)._pendingChildlessStatus = null;
        (Partnership as any)._pendingBroken = false;
        (Partnership as any)._pendingConsangrMode = 'A';

        // assign some properties before drawing so that relationship lines are drawn properly
        // We need to handle these before super()
        if (properties['broken'] !== undefined) {
            (Partnership as any)._pendingBroken = properties['broken'];
        }
        if (properties['consangr'] !== undefined && (properties['consangr'] == 'A' || properties['consangr'] == 'Y' || properties['consangr'] == 'N')) {
            (Partnership as any)._pendingConsangrMode = properties['consangr'];
        }
    ...
    _generateGraphics(x: any, y: any): any {
        if ((Partnership as any)._pendingChildlessStatus !== undefined) {
            this._childlessStatus = (Partnership as any)._pendingChildlessStatus;
            this._type            = 'Partnership';
            this._broken          = (Partnership as any)._pendingBroken;
            this._consangrMode    = (Partnership as any)._pendingConsangrMode;
            delete (Partnership as any)._pendingChildlessStatus;
            delete (Partnership as any)._pendingBroken;
            delete (Partnership as any)._pendingConsangrMode;
        }
     ...
     */
    setConsanguinity(value: any): any {
...
Object.assign(Partnership.prototype, ChildlessBehavior);
